  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-slate-50/50">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-4">
        <StatCard icon={"💰"} label="Amount on Page" value={loading ? '—' : formatCurrency(payments.reduce((sum, p) => sum + (Number(p.amount) || 0), 0))} />
        <StatCard icon={"📊"} label="Transactions on Page" value={loading ? '—' : payments.length} />
      </div>
